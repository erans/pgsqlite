...
impl ObjectResolver {
    ...
    fn get_column_number(
        ...
        let column_info: Vec<(i32, String)> = stmt
            .query_map([], |row| {
                Ok((
...
mod tests {
    ...
    fn test_resolve_column_oid() {
        ...
        let (_table_oid, col_num) =
            ObjectResolver::resolve_column_oid(&conn, "users", "id").unwrap();
    ...
    fn test_get_all_tables() {
        ...
        conn.execute("CREATE TABLE __pgsqlite_metadata (key TEXT)", [])
