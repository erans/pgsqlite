...
static NOW_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(NOW|CURRENT_TIMESTAMP)\s*\(\s*\)").unwrap());

static CURRENT_DATE_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bCURRENT_DATE\b").unwrap());

static CURRENT_TIME_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bCURRENT_TIME\b").unwrap());

// SQLite date/time functions that return text but should be DATE/TIME types
static DATE_FUNCTION_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bdate\s*\(([^)]+)\)").unwrap());

static TIME_FUNCTION_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\btime\s*\(([^)]+)\)").unwrap());

static DATETIME_FUNCTION_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bdatetime\s*\(([^)]+)\)").unwrap());
...
static DATE_TRUNC_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bDATE_TRUNC\s*\(\s*'([^']+)'\s*,\s*([^)]+)\s*\)").unwrap());

static AGE_PATTERN: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bAGE\s*\(").unwrap());
...
impl DateTimeTranslator {
    ...
    pub fn translate_with_metadata(query: &str) -> (String, super::TranslationMetadata) {
        ...
        let is_create_table_with_default =
            query_upper.contains("CREATE TABLE") && query_upper.contains("DEFAULT");
        ...
        if is_create_table_with_default {
            ...
            result = NOW_PATTERN
                .replace_all(&result, "datetime('now')")
                .to_string();
        ...
        result = CURRENT_TIME_PATTERN
            .replace_all(&result, "current_time")
            .to_string();

        // Wrap SQLite date() function to convert to epoch days (INTEGER)
        result = DATE_FUNCTION_PATTERN
        ...
        if parts.len() >= 2 {
            if let Ok(value) = parts[0].parse::<f64>() {
                match parts[1].to_lowercase().as_str() {
                    ...
                    "week" | "weeks" => Some(value * 604_800.0 * 1_000_000.0),
                    "month" | "months" | "mon" | "mons" => Some(value * 2_592_000.0 * 1_000_000.0), // 30 days
                    "year" | "years" | "yr" | "yrs" => Some(value * 31_536_000.0 * 1_000_000.0), // 365 days
                    _ => None,
                }
            } else {
                None
    ...
    fn translate_at_time_zone_with_metadata_impl(
        query: &str,
    ) -> (String, super::TranslationMetadata) {
        let mut metadata = super::TranslationMetadata::new();

        // Pattern to match expressions like "timestamp AT TIME ZONE 'timezone'"
        let result = AT_TIME_ZONE_PATTERN
            .replace_all(query, |caps: &regex::Captures| {
                ...
                if let Some(alias_name) = alias {
                    ...
                    let source_column = if expression
                        .chars()
                ...
                if offset_seconds == 0 {
                    ...
                    format!(
                        "{} + {}{}",
                        expression,
                        offset_seconds as i64 * 1_000_000,
                        alias_part
                    )
                }
            })
...
mod tests {
    ...
    fn test_interval_parsing() {
        assert_eq!(
            DateTimeTranslator::parse_interval_to_seconds("1 second"),
            Some(1_000_000.0)
        );
        assert_eq!(
            DateTimeTranslator::parse_interval_to_seconds("2 minutes"),
            Some(120_000_000.0)
        );
        assert_eq!(
            DateTimeTranslator::parse_interval_to_seconds("3 hours"),
            Some(10_800_000_000.0)
        );
        assert_eq!(
            DateTimeTranslator::parse_interval_to_seconds("1 day"),
            Some(86_400_000_000.0)
        );
        assert_eq!(
            DateTimeTranslator::parse_interval_to_seconds("1 week"),
            Some(604_800_000_000.0)
        );
        assert_eq!(
            DateTimeTranslator::parse_interval_to_seconds("1 month"),
            Some(2_592_000_000_000.0)
        );
        assert_eq!(
            DateTimeTranslator::parse_interval_to_seconds("1 year"),
            Some(31_536_000_000_000.0)
        );
    }
}
