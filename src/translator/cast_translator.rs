use super::{ColumnTypeHint, ExpressionType, SimdCastSearch, TranslationMetadata};
...
impl CastTranslator {
    ...
    pub fn translate_query(query: &str, conn: Option<&Connection>) -> String {
        ...
        if cfg!(debug_assertions) && (query.contains("::mood::text") || query.contains("::text")) {
            tracing::debug!(
                "CastTranslator: translated cast-heavy query (len {} -> {})",
                query.len(),
                result.len()
            );
        }
    ...
    fn extract_cast_metadata(query: &str) -> TranslationMetadata {
        ...
        for caps in SELECT_CAST_PATTERN.captures_iter(query) {
            ...
            if !column_name.is_empty() {
                tracing::debug!(
                    "Cast metadata: {} cast to {} -> column '{}', mapped to PgType {:?}",
                    expr,
                    type_name,
                    column_name,
                    pg_type
            ...
            let translated_cast = if let Some(conn) = conn {
                if Self::is_enum_type(conn, type_name) {
                    ...
                    if clean_expr.contains('(')
                        || Self::is_aggregate_function(clean_expr)
                        || Self::might_need_text_cast(clean_expr)
                    {
        ...
        for type_name in single_word_types {
            if upper_after.starts_with(type_name) {
                ...
                if after.len() == len {
                    ...
                    if *next_char == b'['
                        && after.len() > len + 1
                        && after.as_bytes().get(len + 1) == Some(&b']')
                    {
    ...
    fn translate_cast_syntax(query: &str, conn: Option<&Connection>) -> String {
        ...
        loop {
            ...
            let cast_start_offset = remaining
                .chars()
                .collect::<String>()
                .to_uppercase()
                .find("CAST(");
            ...
            while i < result.len() && paren_count > 0 {
                if result[i..].starts_with('(') {
                    ...
                    && as_pos.is_none()
                    && result[i..].to_uppercase().starts_with(" AS ")
                {
                    as_pos = Some(i);
                }
                i += 1;
