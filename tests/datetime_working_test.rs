...
async fn test_datetime_functions_with_table() {
    ...
    let test_timestamp = 1686840645.0f32; // 2023-06-15 14:30:45 UTC
    client.execute(
...
async fn test_date_trunc_with_table() {
    ...
    // Insert a test timestamp
    let test_timestamp = 1686840645.123456f32; // 2023-06-15 14:30:45.123456 UTC
    client.execute(
        "INSERT INTO timestamps (id, ts) VALUES ($1, $2)",
        &[&1i32, &test_timestamp]
    ).await.unwrap();
    ...
    for msg in results {
        if let tokio_postgres::SimpleQueryMessage::Row(row) = msg {
            ...
            // 2023-06-15 14:00:00
            assert!((hour_val - 1686837600.0).abs() < 1.0);
            // 2023-06-15 00:00:00
            assert!((day_val - 1686787200.0).abs() < 1.0);
            // 2023-06-01 00:00:00
            assert!((month_val - 1685577600.0).abs() < 1.0);
        }
    }
}
...
async fn test_interval_arithmetic_with_table() {
    ...
    let results = client.simple_query(
        "SELECT ts + INTERVAL '1 day' as tomorrow,
                ts - INTERVAL '1 hour' as hour_ago
         FROM timestamps WHERE id = 1"
    ).await.unwrap();
    ...
    for msg in results {
        if let tokio_postgres::SimpleQueryMessage::Row(row) = msg {
            let tomorrow: f64 = row.get(0).unwrap().parse().unwrap();
            let hour_ago: f64 = row.get(1).unwrap().parse().unwrap();
            
            // Verify the calculations
            assert!((tomorrow - (test_timestamp as f64 + 86400.0)).abs() < 1.0); // +1 day
            assert!((hour_ago - (test_timestamp as f64 - 3600.0)).abs() < 1.0);  // -1 hour
        }
    }
}