...
/// pgsqlite keeps its own bookkeeping in tables named `__pgsqlite_*` and
/// materializes some pg_catalog objects as real SQLite tables/views. A client
/// that lists `sqlite_master` (or its `sqlite_schema` alias) directly should not
/// see any of those internal objects, only its own.
///
/// These tests exercise the POST-EXECUTION result filter in the catalog
/// interceptor: the client's query runs verbatim and internal rows are dropped
/// from the result, so the query text is never modified and can never be
/// corrupted.
...
/// Mirrors the private `PG_CATALOG_INTERNAL_TABLES` const in the interceptor.
...
async fn select_star_hides_internal_tables_and_their_indexes() {
    ...
    // the tbl_name predicate in the filter hides indexes/triggers owned by
    // internal tables.
    let rows = server
        .client
        .query("SELECT * FROM sqlite_master ORDER BY name", &[])
        .await
        .expect("SELECT * sqlite_master query should succeed");
    // `name` is the second column of sqlite_master (type, name, tbl_name, ...).
...
/// The critical non-corruption proof. A large `name NOT IN (...)` list of
/// NON-pg names plus an `ESCAPE '\'` clause is exactly the kind of predicate the
/// previous query-rewriting approach corrupted (it returned wrong or zero rows
/// once the list grew past ~16 entries). Because the result filter runs the
/// query VERBATIM, the client's own predicate is honored exactly: the user's
/// tables come back and every excluded name is respected.
#[tokio::test]
async fn large_not_in_list_of_non_pg_names_is_not_corrupted() {
    let server = setup().await;

    // 40 non-pg names, well past the N>=17 threshold where the old code broke.
    // None of these are pgsqlite internals, so nothing here can trip an earlier
    // interceptor branch: the query reaches sqlite_master untouched and the
    // filter simply drops pgsqlite internals from the result.
    let excluded: Vec<String> = (0..40).map(|i| format!("legacy_table_{i:02}")).collect();
    let in_list = excluded
        .iter()
        .map(|n| format!("'{n}'"))
        .collect::<Vec<_>>()
        .join(", ");
    ...
    let rows = server
        ...
        .query(query.as_str(), &[])
        .await
        .expect("large NOT IN query should succeed");
    let names = names(&rows);

    // The regression proof: this MUST return the user's tables, not zero rows.
    assert!(
        names.contains(&"customers".to_string()),
        "customers missing (would be the old corruption bug): {names:?}"
    );
    assert!(names.contains(&"orders".to_string()), "orders missing: {names:?}");
    // The client's own exclusions are respected verbatim: pg_indexes is NOT in
    // the excluded list, so it comes back; none of the excluded names exist so
    // none appear. Internals stay hidden by the result filter.
    assert!(names.contains(&"pg_indexes".to_string()), "pg_indexes missing: {names:?}");
    assert_no_internal(&names);
    for name in &excluded {
        assert!(!names.contains(name), "excluded name leaked: {name}");
    }

    server.abort();
}

/// Documented caveat: `SELECT sql FROM sqlite_master` has no `name` / `tbl_name`
/// column in its projection, so internal rows cannot be identified and the
/// result is returned UNFILTERED. This test pins that behavior so the caveat is
/// explicit and intentional, not an accident.
#[tokio::test]
async fn select_sql_only_is_returned_unfiltered() {
    let server = setup().await;
    let rows = server
        .client
        .query("SELECT sql FROM sqlite_master WHERE sql IS NOT NULL", &[])
        .await
        .expect("SELECT sql query should succeed");

    // The internal probe table's CREATE statement is present precisely because
    // this projection is not filtered. If a future change starts filtering this
    // shape, update the caveat in intercept_sqlite_master_query first.
    let sql_texts: Vec<String> = rows.iter().map(|r| r.get::<_, String>(0)).collect();
    assert!(
        sql_texts.iter().any(|s| s.contains("__pgsqlite_probe")),
