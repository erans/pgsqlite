...
- **Double application is prevented by the `__pgsqlite_` text guard** in `translate`, not by tracking state. If a query passes through both hooks, the second call sees the literal in the rewritten text and returns it untouched.
